package com.android.waylinkage.widget.mulpicture;
...
public class MulPictureAdapter extends BaseAdapter {
    ...
    private List<PictureBean> pictures = new ArrayList<PictureBean>();
    ...
    public MulPictureAdapter(Context context, List<PictureBean> list, OnItemClickClass onItemClickClass, boolean isShowCamera, List<PictureBean> select) {
        this.pictures = list;
    ...
    public View getView(int position, View view, ViewGroup viewGroup) {
        ...
        if (holder != null) {
            PictureBean pictureBean = pictures.get(position - 1);
            for(int i =0; i< select.size();i++){
                if(pictureBean.getLocalURL().equals(select.get(i).getLocalURL())){
                    holder.checkImg.setChecked(true);
                    holder.imageOverLay.setVisibility(View.VISIBLE);
                    break;
                }
            }
            imageLoader.displayImage("file://" + pictureBean.getLocalURL(), holder.imageView, FileUtil.getModelOptions(R.drawable.ic_error_img_bg, 0));
