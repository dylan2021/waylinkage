...
public class PlanProgressDetailActivity extends BaseFgActivity {
    private PlanProgressDetailActivity context;
    private int id;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        initStatusBar();
        setContentView(R.layout.activity_plan_progress_detail);

        context = this;

        id = getIntent().getIntExtra(KeyConstant.id, 0);
        TITLE = getIntent().getStringExtra(KeyConstant.TITLE);
        initTitleBackBt(TITLE + "进度详情");

        getPlanProcessData();
    }

    //计划进度数据
    private void getPlanProcessData() {
        if (!NetUtil.isNetworkConnected(context)) {
            ToastUtil.show(context, getString(R.string.no_network));
            return;
        }
        String url = Constant.WEB_SITE + "/biz/processorPlans/" + id;
        ...
        Request<ProcessorProgressInfo> versionRequest = new
                GsonRequest<ProcessorProgressInfo>(
                    ...
                    public void onErrorResponse(VolleyError volleyError) {
                        ToastUtil.show(context, "获取进度信息失败,稍后重试");
                    }
    ...
    private void setView(ProcessorProgressInfo result) {
        double actualPercentage =  result.getActualPercentage();
        double planPercentage =  result.getPlanPercentage();
        setViewData(R.id.detail_actual_progress_tv, actualPercentage + "%");//实际进度
        setViewData(R.id.progress_actual_plan_comple_value_tv, "(计划" + planPercentage + "%)");//实际进度

        //实际产值
        double actualInvest = result.getActualInvest();
        double planInvest = result.getInvest();//计划产值
        setViewData(R.id.detail_actual_invers_tv, actualInvest + getString(R.string.money_unit));
        setViewData(R.id.inverse_plan_invers_tv,
                "(计划" + planInvest + getString(R.string.money_unit) + ")");
        ...
        String planEndDate = TextUtil.substringTime(result.getPlanEndDate());
